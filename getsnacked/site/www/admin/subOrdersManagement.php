<?
	...
	function subOrdersAdmin() {
		...
		$template->display('admin/subOrdersAdmin.htm');
	} // function subOrdersAdmin
	...
	function viewPackingSlip() {
		...
		if ($subOrder->exists()) {
			...
			$template->assignClean('items', subOrdersController::getSubOrderItems($subOrder->get('subOrderID')));
			ob_end_clean();
			ob_start();
				...
				case 'attention':
					foreach ($records as $subOrderID) {
						...
						if ($subOrder->update()) {
							...
							if (!$subOrder->updateOrderStatus()) {
								addError('There was an error while updating the order status');
							}
						} else {
