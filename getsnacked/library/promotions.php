<?
	...
	class promotions extends dataObject {
		...
		public function applyPromotions() {
			...
			if ($promotionIDs) {
				...
				$package   = $this->dataObject->getPackage('contents');
				$totalCost = $this->dataObject->getPackage('totalCost');
				$order     = $this->dataObject->get('order');
				$orderType = $this->dataObject->get('orderType');
				if (is_array($package) && !empty($package)) {
					...
					if (between($order['quantity'], 1, 999)) {
						$orderQuantity = $order['quantity'];
						$numShips = $orderType == 'subscription' ? $this->dataObject->getArrayData('subscription', 'totalShipments') : 1;
						...
						if ($pArray[$i]['status'] == 'unprocessed') {
							...
							switch ($pArray[$i]['type']) {
								...
								case 'dollartiereddiscount':
									...
									$criteriaT3 = $effect[4];
									$discount3 = $effect[5];
									if (array_key_exists($promoItem, $package)) {
										if ($package[$promoItem]['Q'] * $orderQuantity >= $criteriaT3) { //< 3rd tier
											$pArray[$i]['discount'] = $package[$promoItem]['Q'] * $orderQuantity * $discount3;
										} elseif ($package[$promoItem]['Q'] * $orderQuantity >= $criteriaT2) { //< 2nd tier
											$pArray[$i]['discount'] = $package[$promoItem]['Q'] * $orderQuantity * $discount2;
										} else { //< 1st tier
											$pArray[$i]['discount'] = $package[$promoItem]['Q'] * $orderQuantity * $discount1;
										}
										$pArray[$i]['applied'] = true;
										$pArray[$i]['status'] = 'applied';
									} else {
		...
		private function logPromotionCombo($promoString) {
			...
			if (!$result->rowCount) {
				...
				$this->promotionComboID = $this->promotionComboID['promotionCombination'];
			}
		...
		/**
		 *  Load promotions by promotion combination id
		 *  Args: (int) promotion combination id
		 *  Return: none
		 */
		public function loadPromotionCombo($id) {
			$this->resetPromotions();
			if (validNumber($id, 'integer')) {
				$sql = "SELECT `promotionCombination` FROM `promotionCombination` WHERE `promotionCombinationID` = '".$id."'";
				$result = $this->dbh->query($sql);
				if ($result->rowCount) {
					$row = $result->fetchAssoc();
					$promo_temp = array();
					$promo_temp = explode(';', $row['promotionCombination']);
					foreach ($promo_temp as $key => &$val) {
						$val = clean($val);
						if (empty($val)) unset($promo_temp[$key]);
					}
